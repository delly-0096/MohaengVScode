...
export default function Inquiries() {
  ...
  const fetchInquiries = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:8272/api/admin/inquiry?${params}`,
        {
          method: "GET",
          headers: {
      ...
      setStats({
        ...
        waiting:
          data.dataList?.filter((i) => i.inqryStatus === "waiting").length || 0,
        answered:
          data.dataList?.filter((i) => i.inqryStatus === "answered").length ||
          0,
        today:
          data.dataList?.filter((i) => i.regDt?.startsWith(today)).length || 0,
      });
    } catch (error) {
  ...
  const handleViewDetail = async (inquiry) => {
    try {
      const response = await fetch(
        `http://localhost:8272/api/admin/inquiry/${inquiry.inqryNo}`,
      );
  ...
  const handleAnswerSubmit = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8272/api/admin/inquiry/reply`,
        {
          method: "PUT",
  ...
  const handleDeleteSubmit = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8272/api/admin/inquiry/${deleteModal.inquiry.inqryNo}?alarmCont=${encodeURIComponent(deleteReason)}`,
        {
          method: "DELETE",
        },
      );
  ...
  const handleFileDownload = async (fileNo, fileName) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:8272/api/admin/inquiry/download?fileNo=${fileNo}`,
        {
          method: "GET",
          headers: {
      ...
      {stats.waiting > 0 && (
        ...
        >
          ...
          <span style={{ fontWeight: 500 }}>
            답변 대기 중인 문의가 {stats.waiting}건 있습니다. 빠른 답변
            부탁드립니다.
          </span>
        </div>
      )}
          ...
          return (
            <button
              ...
              style={{
                ...
                borderRadius: 20,
                border: "none",
                background: categoryFilter === cat.id ? "#4A90D9" : "#f1f5f9",
                ...
                inquiriesData.map((inquiry) => (
                  <tr
                    ...
                    style={{
                      ...
                      background:
                        inquiry.inqryStatus === "waiting" ? "#fefce8" : "white",
                    }}
                  >
                    <td style={{ padding: "12px 16px" }}>
                      <span
                        style={{
                          ...
                          fontWeight: 500,
                          background:
                            categoryColors[inquiry.inqryCtgryCd] + "20",
                          color: categoryColors[inquiry.inqryCtgryCd],
                          whiteSpace: "nowrap",
                        }}
                      >
                    ...
                    >
                      {inquiry.regDt
                        ? new Date(inquiry.regDt).toLocaleString("ko-KR")
                        : "-"}
                    </td>
                    <td style={{ padding: "12px 16px", textAlign: "center" }}>
                      <span
                        style={{
                          ...
                          fontWeight: 500,
                          background:
                            statusLabels[inquiry.inqryStatus]?.color + "20",
                          color: statusLabels[inquiry.inqryStatus]?.color,
                        }}
                      >
                        {statusLabels[inquiry.inqryStatus]?.label ||
                          inquiry.inqryStatus}
                      </span>
                    </td>
                    <td style={{ padding: "12px 16px" }}>
                      ...
                      >
                        ...
                        {inquiry.inqryStatus === "waiting" && (
                          <button
                            ...
                            title="답변하기"
                          >
                            <Reply size={16} style={{ color: "#1e40af" }} />
                          </button>
                        )}
                        <button
            ...
            return (
              <button
                ...
                style={{
                  padding: "8px 12px",
                  border: "1px solid #e2e8f0",
                  background: currentPage === page ? "#4A90D9" : "white",
          ...
          detailModal.inquiry && (
            <div style={{ display: "flex", gap: 8 }}>
              ...
              <button
                onClick={() => {
                  setDetailModal({ isOpen: false, inquiry: null });
                  handleDelete(detailModal.inquiry);
                }}
                style={{
                  padding: "10px 20px",
                  background: "#ef4444",
          ...
          <div>
            ...
            >
              <span
                style={{
                  ...
                  fontWeight: 500,
                  background:
                    categoryColors[detailModal.inquiry.inqryCtgryCd] + "20",
                  color: categoryColors[detailModal.inquiry.inqryCtgryCd],
                }}
              >
                {detailModal.inquiry.categoryName}
              </span>
              <span
                style={{
                  padding: "4px 10px",
                  ...
                  fontWeight: 500,
                  background:
                    statusLabels[detailModal.inquiry.inqryStatus]?.color + "20",
                  color: statusLabels[detailModal.inquiry.inqryStatus]?.color,
                }}
              >
                {statusLabels[detailModal.inquiry.inqryStatus]?.label}
              </span>
            </div>
            <div
              style={{
            ...
            >
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <span style={{ fontWeight: 500 }}>
                  {detailModal.inquiry.memberName}
                </span>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <span style={{ fontWeight: 500 }}>
                  {detailModal.inquiry.inqryEmail}
                </span>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <span style={{ fontWeight: 500 }}>
                  {detailModal.inquiry.regDt
                    ? new Date(detailModal.inquiry.regDt).toLocaleString(
                        "ko-KR",
                      )
                    : "-"}
                </span>
              ...
              {detailModal.inquiry.inquiryTargetNo && (
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <Ticket size={16} style={{ color: "#64748b" }} />
                  <span style={{ color: "#64748b", fontSize: "0.875rem" }}>
                    예약번호
                  </span>
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              >
                <HelpCircle size={18} style={{ color: "#4A90D9" }} />
              ...
              >
                {detailModal.inquiry.inqryTitle}
              </h4>
              <div
                style={{
                  padding: 16,
              ...
              >
                {detailModal.inquiry.inqryCn}
              </div>
              {detailModal.inquiry.attachFiles &&
                detailModal.inquiry.attachFiles.length > 0 && (
                  <div style={{ marginTop: 12 }}>
                    ...
                    >
                      ...
                      <span style={{ fontSize: "0.875rem", color: "#64748b" }}>
                        첨부파일 ({detailModal.inquiry.attachFiles.length})
                      </span>
                    </div>
            ...
            {detailModal.inquiry.replyCn && (
              <div>
                ...
                >
                  ...
                  >
                    <Reply size={18} style={{ color: "#10b981" }} />
                    <span style={{ fontWeight: 600, color: "#10b981" }}>
                      답변
                    </span>
                  </div>
                  <span style={{ fontSize: "0.875rem", color: "#64748b" }}>
                    {detailModal.inquiry.replyDt
                      ? new Date(detailModal.inquiry.replyDt).toLocaleString(
                          "ko-KR",
                        )
                      : ""}
                  </span>
        ...
        footer={
          <>
            ...
            <button
              ...
              style={{
                padding: "10px 20px",
                background: !answerText.trim() ? "#cbd5e1" : "#4A90D9",
                color: "white",
                border: "none",
                borderRadius: 8,
        ...
        {answerModal.inquiry && (
          <div>
            ...
            >
              ...
              >
                <span
                  style={{
                    ...
                    fontWeight: 500,
                    background:
                      categoryColors[answerModal.inquiry.inqryCtgryCd] + "20",
                    color: categoryColors[answerModal.inquiry.inqryCtgryCd],
                  }}
                >
                  {answerModal.inquiry.categoryName}
                </span>
                <span style={{ color: "#64748b", fontSize: "0.875rem" }}>
                  {answerModal.inquiry.memberName} |{" "}
                  {answerModal.inquiry.regDt
                    ? new Date(answerModal.inquiry.regDt).toLocaleString(
                        "ko-KR",
                      )
                    : ""}
                </span>
              ...
              >
                {answerModal.inquiry.inqryTitle}
              </h4>
              <p
                style={{
                  color: "#64748b",
            ...
            <div>
              ...
              />
              <div
                style={{ marginTop: 8, fontSize: "0.875rem", color: "#64748b" }}
              >
                답변은 고객의 이메일({answerModal.inquiry.inqryEmail})로 자동
                발송됩니다.
              </div>
            </div>
          </div>
        ...
        footer={
          <>
            ...
            <button
              ...
              style={{
                padding: "10px 20px",
                background: !deleteReason.trim() ? "#fca5a5" : "#ef4444",
                color: "white",
                border: "none",
                borderRadius: 8,
        ...
        {deleteModal.inquiry && (
          <div>
            ...
            >
              ...
              >
                삭제된 문의는 복구할 수 없으며, 작성자에게 삭제 사유가 포함된
                알림이 전송됩니다.
              </p>
            </div>
            <div
            ...
            >
              <h4
                style={{ marginBottom: 8, fontSize: "1rem", fontWeight: 600 }}
              >
                {deleteModal.inquiry.inqryTitle}
              </h4>
              <div style={{ color: "#64748b", fontSize: "0.875rem" }}>
                {deleteModal.inquiry.memberName} |{" "}
                {deleteModal.inquiry.regDt
                  ? new Date(deleteModal.inquiry.regDt).toLocaleString("ko-KR")
                  : ""}
              </div>
            </div>
            <div>
              ...
              <textarea
                ...
                placeholder="삭제 사유를 입력하세요. 이 내용은 작성자에게 알림으로 전송됩니다."
                style={{
                  width: "100%",
                  minHeight: 120,
                  padding: 12,
