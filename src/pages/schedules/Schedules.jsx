...
function Schedules() {
  ...
  const getCategoryIcon = (categoryName) => {
    ...
    switch (categoryName) {
      ...
      case "맛집":
        return "bi-cup-hot";
      default:
        return "bi-pin"; // 매칭되는 게 없을 때 기본 아이콘
    }
  };

  // const getCategoryIcon = (contentTypeId) => {
  //   // 숫자로 들어올 경우를 대비해 문자열 변환 처리
  //   switch (String(contentTypeId)) {
  //     case "12": // 관광지
  //       return "bi-geo-alt";
  //     case "14": // 문화시설
  //       return "bi-building-columns"; // 박물관, 미술관 느낌
  //     case "15": // 행사/축제
  //       return "bi-calendar-event";
  //     case "25": // 여행코스
  //       return "bi-signpost-split";
  //     case "28": // 레포츠
  //       return "bi-bicycle"; // 또는 bi-activity
  //     case "32": // 숙박
  //       return "bi-house";
  //     case "38": // 쇼핑
  //       return "bi-bag";
  ...
  const filteredData = schedulesData.filter((schedule) => {
    ...
    const matchesDestination =
      filterDestination === "all" || schedule.destination === filterDestination;
  ...
  const getTotalEstimatedCost = (dailySchedule) => {
    ...
    return dailySchedule.reduce((total, day) => {
      return (
        total +
        day.places.reduce(
          (dayTotal, place) => dayTotal + (place.estimatedCost || 0),
          0
        )
      );
    }, 0);
  ...
  useEffect(() => {
    ...
    let schudlList = [];
    fetch("http://localhost:8272/api/admin/schedule/list", {
      method: "POST",
      headers: {
      ...
      .then((res) => res.json())
      .then((datas) => {
        console.log("일정 목록 데이터:", datas);
        let dataList = datas.scheduleList;
        dataList.forEach((data) => {
          let resData = {
            ...
            endDate: data.schdlStartDt,
            status: data.schdlStsNm,
            isPublic: true,
            isAIRecommended: data.aiRecomYn == "Y" ? true : false,
          ...
          detailsList.forEach((detail) => {
            let day = detail.schdlDt;
            let date = detail.schdlStartDt;
            let places = [];

            let placesList = detail.tripSchedulePlaceList;
            placesList.forEach((place) => {
              console.log("place:", place.tourPlace.placeTypeName);
              places.push({
                time: place.placeStartTime,
                category: place.tourPlace.placeTypeName,
                name: place.plcNm,
                location: "제주시 연동",
                rating: 4.5,
                estimatedCost: place.planCost,
              });
            });
            details.push({
              day: day,
          ...
          resData.dailySchedule = details;

          schudlList.push(resData);
        });
        setSchedulesData(schudlList);
      });
  }, []);
      ...
      <div className="filter-section">
        <div className="filter-row">
          ...
          <div className="filter-group">
            ...
            <select
              value={filterTravelLog}
              onChange={(e) => setFilterTravelLog(e.target.value)}
            >
            ...
            {filteredData.map((schedule) => (
              <tr key={schedule.id}>
                <td>
                  <div className="schedule-title-cell">
                    ...
                    <small className="text-muted">
                      {schedule.createdAt} 등록 · 최근 수정:{" "}
                      {schedule.lastModified?.split(" ")[0] ||
                        schedule.createdAt}
                    </small>
                  </div>
                </td>
                ...
                <td>
                  ...
                  <small className="text-muted">
                    {getDuration(schedule.startDate, schedule.endDate)}
                  </small>
                </td>
                <td>
                  <span className="places-count">
                    <i className="bi bi-geo-alt"></i>{" "}
                    {getPlacesCount(schedule.dailySchedule)}곳
                  </span>
                </td>
                <td>
                  <button
                  ...
                  >
                    <i
                      className={`bi ${
                        schedule.isPublic ? "bi-globe" : "bi-lock"
                      }`}
                    ></i>
                  </button>
                </td>
                <td>
                  <div className="view-share-stats">
                    <span title="조회수">
                      <i className="bi bi-eye"></i> {schedule.views}
                    </span>
                    <span title="공유수">
                      <i className="bi bi-share"></i> {schedule.shareCount}
                    </span>
                  </div>
                </td>
                <td>
            ...
            <div className="modal-body">
                ...
                <div className="schedule-meta">
                  ...
                  <span
                    className={`visibility-badge ${
                      selectedSchedule.isPublic ? "public" : "private"
                    }`}
                  >
                    <i
                      className={`bi ${
                        selectedSchedule.isPublic ? "bi-globe" : "bi-lock"
                      }`}
                    ></i>
              ...
              <div className="detail-grid">
                ...
                <div className="detail-section">
                  <h4>
                    <i className="bi bi-info-circle"></i> 기본 정보
                  </h4>
                  ...
                  <div className="detail-row">
                    ...
                    <span className="value">
                      {selectedSchedule.companions}명
                    </span>
                  </div>
                  <div className="detail-row">
                    <span className="label">예산</span>
                    <span className="value">
                      {formatPrice(selectedSchedule.budget)}
                    </span>
                  </div>
                  <div className="detail-row">
                    ...
                    <span className="value">
                      {formatPrice(
                        getTotalEstimatedCost(selectedSchedule.dailySchedule)
                      )}
                    </span>
                  </div>
                </div>
                ...
                <div className="detail-section">
                  <h4>
                    <i className="bi bi-person"></i> 작성자 정보
                  </h4>
                  <div className="detail-row">
                  ...
                  <div className="detail-row">
                    ...
                    <span className="value">
                      {selectedSchedule.memberEmail}
                    </span>
                  </div>
                  <div className="detail-row">
                    <span className="label">연락처</span>
                    <span className="value">
                      {selectedSchedule.memberPhone}
                    </span>
                  </div>
                </div>
                ...
                <div className="detail-section">
                  <h4>
                    <i className="bi bi-bar-chart"></i> 통계
                  </h4>
                  <div className="detail-row">
                    <span className="label">방문 장소</span>
                    <span className="value">
                      {getPlacesCount(selectedSchedule.dailySchedule)}곳
                    </span>
                  </div>
                  <div className="detail-row">
                ...
                {selectedSchedule.status === "완료" && (
                  <div className="detail-section">
                    <h4>
                      <i className="bi bi-journal-text"></i> 여행기록
                    </h4>
                    <div className="detail-row">
                ...
                {selectedSchedule.status === "취소" && (
                  <div className="detail-section">
                    <h4>
                      <i className="bi bi-x-circle"></i> 취소 정보
                    </h4>
                    <div className="detail-row">
                ...
                <div className="detail-section full-width">
                  <h4>
                    <i className="bi bi-calendar-day"></i> 일별 상세 일정
                  </h4>
                  ...
                  {/* 선택된 날짜의 일정 */}
                  {selectedSchedule.dailySchedule?.find(
                    (d) => d.day === selectedDay
                  ) && (
                    <div className="day-timeline">
                      {selectedSchedule.dailySchedule
                        ...
                        .places.map((place, index) => (
                          <div
                            key={index}
                            className={`timeline-item ${getCategoryClass(
                              place.category
                            )}`}
                          >
                              ...
                              <div className="timeline-details">
                                ...
                                {place.estimatedCost > 0 && (
                                  <span className="cost">
                                    ...
                                    {formatPrice(place.estimatedCost)}
                                  </span>
                                )}
                              </div>
                            </div>
                ...
                {selectedSchedule.reservations?.length > 0 && (
                  <div className="detail-section full-width">
                    <h4>
                      <i className="bi bi-bookmark-check"></i> 연결된 예약
                    </h4>
                    <div className="reservations-list">
                      {selectedSchedule.reservations.map(
                        (reservation, index) => (
